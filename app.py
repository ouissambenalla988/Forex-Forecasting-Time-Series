...
if os.path.exists(secrets_path):
    ...
    ALPHA_VANTAGE_API_KEY = secrets.get('ALPHA_VANTAGE', {}).get('key', None)
    NEWS_API_KEY = secrets.get('NEWS_API', {}).get('key', None)
else:
...
st.markdown("""
This dashboard uses a <a href="https://facebook.github.io/prophet/" target="_blank" style="text-decoration: none; color: #1f77b4;">Prophet Facebook</a> model to predict forex price movements using Alpha Vantage data.  
The default analysis period is 100 days.
...
selected_pair = st.sidebar.selectbox(
 ...
 # Remplacer les barres obliques pour correspondre à l'URL de TradingView
selected_pair_for_url = selected_pair.replace("/", "")
# Timeframe selection
...
try:
    ...
    # Create tabs
    tab1, tab2, tab3, tab4 = st.tabs(["Market Trend View", "Technical Indicators", "Predictions", f"Forex News of the {selected_pair}"])
    
    with tab1:
        st.subheader("TradingView Chart")
        
         # Candlestick chart
        # Intégrer le widget TradingView via iframe
        tv_advanced_chart = f"""
            <!-- TradingView Advanced Chart BEGIN -->
            ...
            <!--TradingView Advanced Chart END -->
            """

        # Intégrer le widget dans Streamlit (page principale)
        components.html(tv_advanced_chart, height=630, scrolling=True)
    ...
    with tab2:
        ...
        st.subheader(f"📺 TradingView Chart - {selected_pair} with Indicators")
        ...
        tradingview_html = f"""
        <div class="tradingview-widget-container">
        <div id="tradingview_{selected_pair_for_url}"></div>
        <script type="text/javascript" src="https://s3.tradingview.com/tv.js"></script>
        <script type="text/javascript">
        new TradingView.widget({{
            "width": "105%",
            ...
            "container_id": "tradingview_{selected_pair_for_url}"
        }});
        </script>
        </div>
        """

        # Inject in Streamlit app
...
body[data-theme="light"] .name-links a {
    color: #333;
}

body[data-theme="light"] .name-links {
    background: #f9f9f9;
}

body[data-theme="light"] .name-links a:hover {
    color: #0056b3;
}
...
body[data-theme="dark"] .name-links a {
    color: #f0f2f6;
}

body[data-theme="dark"] .name-links {
    background: #1e1e1e;
}

body[data-theme="dark"] .name-links a:hover {
    color: #66b2ff;
}
...
.name-container:hover .name-links {
    visibility: visible;
    opacity: 1;
    pointer-events: auto;
...
.footer a:hover {
    color: #0077cc;  /* Couleur plus vive */
